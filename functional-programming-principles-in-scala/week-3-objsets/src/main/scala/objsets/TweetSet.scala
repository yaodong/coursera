...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    def findMax(acc: Tweet, b: Tweet): Tweet =
      if (acc.retweets > b.retweets) acc else b
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = allTweets.filter(p => google.exists(p.text.contains(_)))
  lazy val appleTweets: TweetSet = allTweets.filter(p => apple.exists(p.text.contains(_)))
