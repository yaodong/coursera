...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    for {
      (k, v) <- w.toLowerCase.toCharArray.groupBy(identity)
    } yield (k, v.length)
  }.toList.sortBy(_._1)
  ...
  def wordAnagrams(word: Word): List[Word] =
    dictionaryByOccurrences.find(_._2.contains(word)) match {
      case Some(v) => v._2
      case None => List()
    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val ym = y.toMap
    x.map(occ => (occ._1, occ._2 - ym.getOrElse(occ._1, 0))).filter(_._2 > 0)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def iter(occ: Occurrences): List[Sentence] = {
      if (occ.isEmpty) List(Nil)
        ...
        sentence <- iter(subtract(occ, wordOccurrences(word)))
        if comb.nonEmpty
      } yield word :: sentence
    }
